...
// Initialization of the log functions
	logger->initLogger("measurement_LevelController.csv", "log.txt",
			measurementNames);
...
// Instantiation of the FMU
	...
	s1 = f_instantiate("LevelController", fmi2CoSimulation,
			"{d6f9df54-1689-46cc-b366-77379034dd53}", "", &cbf, fmi2True,
			fmi2False);
	if (s1 == NULL) {
		logger->fmiLog(NULL, "LevelController", fmi2Error, "error",
				"fmi2Instatiate is not created.");
	} else {
		logger->fmiLog(NULL, "LevelController", fmi2OK, "info",
...
void FMI_LevelController::execute() {
	...
	/*
	 *
	 if true, needs to rollback
	 One simulation step will be divided into two sub-steps:
	 Store model internal state, update model internal state without advancing simulation time, update output to others
	 Restore model internal state, retrieve updated input from others, update model internal state with advancing simulation time, update output to others
	 *
	 */
	...
	//without roll back mechanism
		...
		if (simulationFinished == false && status == fmi2OK) {
			...
			if (currentTime < 20.0) {
				...
				// Save the values of the input and output to a file
				logger->saveMeasurementPoint(
						std::to_string(currentTime) + "," + std::to_string(in)
								+ "," + std::to_string(out));
