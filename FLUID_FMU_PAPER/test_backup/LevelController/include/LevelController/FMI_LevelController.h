/**
 * Header file for the FMI_LevelController model
 * Generated by the TERRA CSPm2LUNA generator version 1.1.3
 *
 * FMI C++ glue code to connect to LevelController
 * Modifying this file is absolutely not recommended!
 *
 * protected region document description on begin
 *
 * protected region document description end
 */

#pragma once

#include "csp/CSP.h"
#include "fmi2Functions.h" 
#include "logger.h" 
// protected region additional headers on begin
// Each additional header should get a corresponding dependency in the Makefile
// protected region additional headers end

using namespace LUNA::CSP;

namespace LevelController { namespace FMI_LevelController { 

class FMI_LevelController : public CodeBlock
{

public:
  // Define constructor and destructor
  FMI_LevelController(double &in, double &out, fmi2FMUstate &FMUstate);
  virtual ~FMI_LevelController();

  void execute();

private:

  // Class variables
  double &in;
  double &out;
  fmi2FMUstate &FMUstate;


// Make a new FMU component
fmi2Component s1 = NULL;


//FMIlogger declaration
FMILogger_LevelController *logger = new FMILogger_LevelController();

// functions for the FMU
fmi2CallbackFunctions cbf = {FMILogger_LevelController::fmiLog,calloc,free,NULL,NULL};

// variables for the simulation.
double timeStep = 0;
double currentTime = 0;
fmi2Status status = fmi2OK;
bool simulationFinished = false;

// typedef for loading shared lib functions
typedef fmi2Component (*fp_instantiate)(fmi2String, fmi2Type, fmi2String, fmi2String, const fmi2CallbackFunctions*, fmi2Boolean, fmi2Boolean);
typedef fmi2Status (*fp_setup)(fmi2Component, fmi2Boolean, fmi2Real, fmi2Real, fmi2Boolean, fmi2Real);
typedef fmi2Status (*fp_exit)(fmi2Component);
typedef fmi2Status (*fp_doStep)(fmi2Component, fmi2Real, fmi2Real, fmi2Boolean);
typedef fmi2Status (*fp_enterInit)(fmi2Component);

// pointers for loading shared lib functions
void *handle;
fp_instantiate f_instantiate;
fp_setup f_setup;
fp_exit f_exit;
fp_doStep f_doStep;
fp_enterInit f_enterInit;
};

// Close namespace(s)
} } 
